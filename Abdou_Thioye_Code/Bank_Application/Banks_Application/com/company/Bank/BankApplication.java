...
public class BankApplication {
			...
			while (loggedIn) {
				...
				for (Account acc : myAccounts) {
					if ((acc.firstName.equals(firstName) /* || acc.lastName.equals(lastName) */)
						...
						System.out.println("");
						System.out.println("Welcome To Your Bank Accout " + firstName.toUpperCase() + "!");
						System.out.println("");
						int choice;
						choice = menu();
	...
	public static void MainLogin1() {
		System.out.println("Welcome! \nPlease press 1 to login As an Employee or 2 to login as a Customer");
	}
	...
	public static ArrayList FileIn() throws IOException {
		...
		try {
			reader = new BufferedReader(new FileReader("Bank_Applications/Resources/UserFile"));
			String line;
			int count = 0;
	...
	public static void FileOut(ArrayList myAccounts) throws IOException {
		...
		try {
			writer = new FileWriter("Bank_Applications/Resources/UserFile");
	...
	public static void NewAccount() throws IOException {
		...
		// Files.write(Paths.get("Bank_Applications/Resources/Accounts"),
	...
	public static void Employee() throws IOException {
		...
		try {
			reader = new BufferedReader(new FileReader("Bank_Applications/Resources/EmployeeFile"));
			String line;
			int count = 0;
			while ((line = reader.readLine()) != null) {
				count++;
				...
				FileIn1();
				System.out.println("Here are the accounts. Please select one of the following number");
				...
				while (true) {
					...
					System.out.println("");
					System.out.println("1.) Delet an account");
					System.out.println("2.) Edit an account ");
					System.out.println("3.) Canceling accounts");
					System.out.println("4.) View all the accounts Information");
					System.out.println("5.) Log out");
					// Checks for user input makes sure its a integer , This is for the menu
					Scanner sc1 = new Scanner(System.in);

					try {
						choice3 = Integer.parseInt(sc1.next());
						break; // will only get to here if input is int
						// into not then it throws out the custom exception
					} catch (NumberFormatException ignore) {
						System.out.println("Invalid input, Select the correct numeric value for menu item.");
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				System.out.println(count + ". First: " + data[0].toUpperCase()+ "\t" + " | Last: " + data[1].toUpperCase()
						+  "\t" + " | Pin#: " + data[2] +"\t" + " | Account Ballance: " + data[3] +"\t" + " | Account Last Transaction: "
						+ data[4]);
				myAccounts.add(userAccount);
			}
			System.out.println("\n" + "Your Total Accounts in File:--> " + count);
		} catch (IOException ex) {
			ex.printStackTrace();
		} finally {
			if (reader != null) {
